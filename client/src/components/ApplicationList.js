...
function ApplicationList({ applications, onStatusUpdate }) {
  ...
  const handleStatusUpdate = async (id, newStatus) => {
    try {
      await axios.post(`${API_BASE}/applications/${id}/status`, {
        status: newStatus,
      });
      onStatusUpdate();
    } catch (error) {
  ...
  const handleRegenerateCoverLetter = async id => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/applications/${id}/regenerate-cover-letter`
      );
  ...
  return (
    <div>
              ...
              secondary={
                <div>
                  ...
                  <Button
                    ...
                    startIcon={
                      regenerating && regeneratingId === app.workflow_id ? (
      ...
      >
        <DialogTitle>
          Cover Letter - {selectedApplication?.role} at{' '}
